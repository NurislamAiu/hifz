...
class ProgressController extends Notifier<Map<String, MemorizationStatus>> {
  ...
  Map<String, MemorizationStatus> build() {
    ...
    return {for (final p in repo.getAll()) '${p.surahNumber}:${p.ayahNumberInSurah}': p.status};
  }
  ...
  Future<void> setStatus(int surahNumber, int ayahNumberInSurah, MemorizationStatus status) async {
    await ref.read(progressRepositoryProvider).setStatus(surahNumber, ayahNumberInSurah, status);
...
final progressControllerProvider =
    NotifierProvider<ProgressController, Map<String, MemorizationStatus>>(ProgressController.new);
...
final overallProgressProvider = Provider<double>((ref) {
  ...
  final memorized = statuses.values.where((s) => s == MemorizationStatus.memorized).length;
  return memorized / totalAyahs;
});

final surahProgressPercentProvider = Provider.family<double, (int surahNumber, int ayahCount)>((ref, args) {
  final (surahNumber, ayahCount) = args;
  if (ayahCount == 0) return 0;
  final statuses = ref.watch(progressControllerProvider);
  var memorized = 0;
  for (var i = 1; i <= ayahCount; i++) {
    if (statuses['$surahNumber:$i'] == MemorizationStatus.memorized) memorized++;
  }
...
final juzProgressProvider = Provider<Map<int, double>>((ref) {
  ...
  for (final ayah in quranRepo.getAllAyahs()) {
    ...
    if (statuses['${ayah.surahNumber}:${ayah.numberInSurah}'] == MemorizationStatus.memorized) {
      memorizedByJuz[ayah.juz] = (memorizedByJuz[ayah.juz] ?? 0) + 1;
    }
  }
  return {for (final juz in totalsByJuz.keys) juz: (memorizedByJuz[juz] ?? 0) / totalsByJuz[juz]!};
});
